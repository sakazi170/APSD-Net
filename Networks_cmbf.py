...
from modules.CMBF import CMBF, CMBF_v1, CMBF_v2, CMBF_v3
...
class LeadingEncoder(nn.Module):
    ...
    def forward(self, x, g1=None, g2=None, g3=None):
        ...
        f1 = self.layer1(f0)
        if g1 is not None:
            f1 = f1 + g1  # Simple addition instead of FreqLGFA
        x = self.pool1(f1)

        # Layer 2 with simple addition
        f2 = self.layer2(x)
        if g2 is not None:
            f2 = f2 + g2  # Simple addition instead of FreqLGFA
        x = self.pool2(f2)

        # Layer 3 with simple addition
        f3 = self.layer3(x)
        if g3 is not None:
            f3 = f3 + g3  # Simple addition instead of FreqLGFA
        x = self.pool3(f3)
...
class BTS_CMBF(nn.Module):
    """
    BTS model with CMBF only (no FreqLGFA).
    - Simple addition at layers 1, 2, 3 for cross-modal fusion.
    - CMBF at bottleneck for advanced cross-modal fusion.
    """

    def __init__(self, patch_size_d=None, patch_size_h=None, patch_size_w=None,
                 in_channels=1, num_classes=4, num_heads=8, num_points=4):
        super().__init__()
        ...
        # CMBF at bottleneck (8×8×8)
        self.cmbf = CMBF(channels=256, num_heads=num_heads, num_points=num_points)
    ...
    def forward(self, t1, t1ce, t2, flair):
        ...
        # CMBF fusion at bottleneck
        bottleneck_fused = self.cmbf(f_local=f4, f_global=g4)
...
class BTS_CMBF1(nn.Module):
    ...
    def __init__(self, patch_size_d=None, patch_size_h=None, patch_size_w=None,
        ...
        # CMBF at bottleneck (8×8×8)
        self.cmbf_v1 = CMBF_v1(channels=256, num_heads=num_heads, num_points=num_points)
    ...
    def forward(self, t1, t1ce, t2, flair):
        ...
        # CMBF fusion at bottleneck
        bottleneck_fused = self.cmbf_v1(f_local=f4, f_global=g4)
        # bottleneck_fused: [B, 256, 8, 8, 8]
        output = self.decoder(bottleneck_fused, f0, f1, f2, f3)
        # output: [B, num_classes, 128, 128, 128]
        return output

class BTS_CMBF2(nn.Module):
    ...
    def __init__(self, patch_size_d=None, patch_size_h=None, patch_size_w=None,
        ...
        # CMBF at bottleneck (8×8×8)
        self.cmbf_v2 = CMBF_v2(channels=256, num_heads=num_heads, num_points=num_points)
        self.decoder = Decoder(num_classes=num_classes)
        self.apply(self._init_weights)
    ...
    def forward(self, t1, t1ce, t2, flair):
        ...
        # CMBF fusion at bottleneck
        bottleneck_fused = self.cmbf_v2(f_local=f4, f_global=g4)
        # bottleneck_fused: [B, 256, 8, 8, 8]
        output = self.decoder(bottleneck_fused, f0, f1, f2, f3)
        # output: [B, num_classes, 128, 128, 128]
        return output

class BTS_CMBF3(nn.Module):
    ...
    def __init__(self, patch_size_d=None, patch_size_h=None, patch_size_w=None,
        ...
        self.cmbf_v3 = CMBF_v3(channels=256, num_heads=num_heads, num_points=num_points)
        self.decoder = Decoder(num_classes=num_classes)
        self.apply(self._init_weights)
    ...
    def forward(self, t1, t1ce, t2, flair):
        ...
        # CMBF fusion at bottleneck
        bottleneck_fused = self.cmbf_v3(f_local=f4, f_global=g4)
        output = self.decoder(bottleneck_fused, f0, f1, f2, f3)
        return output
...
def test_model():
    ...
    print("=" * 80)
    num_classes = 4
    model = BTS_CMBF(128,128,128, in_channels=1, num_classes=num_classes)
